# Emissão de passaportes de permissão via Civic Pass

## Diagnóstico
**Contexto Operacional:** Emissão de passaportes de permissão via Civic Pass.
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-skill-32`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Emissão de passaportes de permissão via Civic Pass...");
    const executionResult = await performskill32Logic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-skill-32",
        status: "SUCCESS",
...
async function performskill32Logic(payload: any) {
