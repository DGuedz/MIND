...
## Diagnóstico
**Contexto Operacional:** Configuração de cofres (Vaults) multi-layer de Produtos Estruturados (Crédito/Opções).
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-vaults`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Configuração de cofres...");
    const executionResult = await performvaultsLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-vaults",
        status: "SUCCESS",
...
async function performvaultsLogic(payload: any) {
