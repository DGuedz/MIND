# Configuração ágil de RPCs e DAS APIs para indexação de NFTs

## Diagnóstico
**Contexto Operacional:** Configuração ágil de RPCs e DAS APIs para indexação de NFTs (Helius / QuickNode).
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-helius---quicknode`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Configuração ágil de RPCs e DAS APIs para indexação de NFTs...");
    const executionResult = await performheliusquicknodeLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-helius---quicknode",
        status: "SUCCESS",
...
async function performheliusquicknodeLogic(payload: any) {
