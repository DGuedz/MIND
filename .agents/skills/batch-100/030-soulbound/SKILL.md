# Emissão e verificação de Identidades via Tokens Intransferíveis

## Diagnóstico
**Contexto Operacional:** Emissão e verificação de Identidades via Tokens Intransferíveis (Soulbound).
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-soulbound`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Emissão e verificação de Identidades via Tokens Intransferíveis...");
    const executionResult = await performsoulboundLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-soulbound",
        status: "SUCCESS",
...
async function performsoulboundLogic(payload: any) {
