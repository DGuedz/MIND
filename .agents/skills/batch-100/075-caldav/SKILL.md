# Sincronização e consultas avançadas a calendários

## Diagnóstico
**Contexto Operacional:** Sincronização e consultas avançadas a calendários (Caldav).
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-caldav`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Sincronização e consultas avançadas a calendários...");
    const executionResult = await performcaldavLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-caldav",
        status: "SUCCESS",
...
async function performcaldavLogic(payload: any) {
