# Lançamento de Mercados de Ações Pré-IPO

## Diagnóstico
**Contexto Operacional:** Lançamento de Mercados de Ações Pré-IPO (Xstocks) sob KYC restrito.
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-xstocks`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Lançamento de Mercados de Ações Pré-IPO...");
    const executionResult = await performxstocksLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-xstocks",
        status: "SUCCESS",
...
async function performxstocksLogic(payload: any) {
