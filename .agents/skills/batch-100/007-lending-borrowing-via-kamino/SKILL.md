# Operações de empréstimo e tomada de crédito

## Diagnóstico
**Contexto Operacional:** Operações de empréstimo e tomada de crédito (Lending/Borrowing via Kamino).
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-lending-borrowing-via-kamino`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Operações de empréstimo e tomada de crédito...");
    const executionResult = await performlendingborrowingviakaminoLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-lending-borrowing-via-kamino",
        status: "SUCCESS",
...
async function performlendingborrowingviakaminoLogic(payload: any) {
