# Rastreamento de P&L de carteiras e descoberta de tokens

## Diagnóstico
**Contexto Operacional:** Rastreamento de P&L de carteiras e descoberta de tokens (Birdeye).
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-birdeye`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Rastreamento de P&L de carteiras e descoberta de tokens...");
    const executionResult = await performbirdeyeLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-birdeye",
        status: "SUCCESS",
...
async function performbirdeyeLogic(payload: any) {
