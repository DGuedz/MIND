# Participação automatizada em lançamentos de tokens

## Diagnóstico
**Contexto Operacional:** Participação automatizada em lançamentos de tokens (Raydium LaunchLab).
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-raydium-launchlab`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Participação automatizada em lançamentos de tokens...");
    const executionResult = await performraydiumlaunchlabLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-raydium-launchlab",
        status: "SUCCESS",
...
async function performraydiumlaunchlabLogic(payload: any) {
