...
## Diagnóstico
**Contexto Operacional:** Bloqueio de reversão (Rollback garantido) caso a liquidação do Card falhe atômicamente.
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-rollback-garantido`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Bloqueio de reversão...");
    const executionResult = await performrollbackgarantidoLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-rollback-garantido",
        status: "SUCCESS",
...
async function performrollbackgarantidoLogic(payload: any) {
