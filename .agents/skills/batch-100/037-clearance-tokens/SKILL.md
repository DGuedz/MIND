# Gerenciamento de Tokens de Capacidade

## Diagnóstico
**Contexto Operacional:** Gerenciamento de Tokens de Capacidade (Clearance Tokens) para acessos militares/privados.
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-clearance-tokens`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Gerenciamento de Tokens de Capacidade...");
    const executionResult = await performclearancetokensLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-clearance-tokens",
        status: "SUCCESS",
...
async function performclearancetokensLogic(payload: any) {
