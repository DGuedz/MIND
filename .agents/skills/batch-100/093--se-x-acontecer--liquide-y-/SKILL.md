# Lançamento de Mercados Condicionais em escala

## Diagnóstico
**Contexto Operacional:** Lançamento de Mercados Condicionais em escala ("Se X acontecer, liquide Y").
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill--se-x-acontecer--liquide-y-`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Lançamento de Mercados Condicionais em escala...");
    const executionResult = await performsexacontecerliquideyLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill--se-x-acontecer--liquide-y-",
        status: "SUCCESS",
...
async function performsexacontecerliquideyLogic(payload: any) {
