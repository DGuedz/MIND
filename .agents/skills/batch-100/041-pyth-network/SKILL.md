# Consulta de preços de ativos em tempo real

## Diagnóstico
**Contexto Operacional:** Consulta de preços de ativos em tempo real (Pyth Network).
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-pyth-network`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Consulta de preços de ativos em tempo real...");
    const executionResult = await performpythnetworkLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-pyth-network",
        status: "SUCCESS",
...
async function performpythnetworkLogic(payload: any) {
