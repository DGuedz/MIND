# Ranqueamento de credibilidade de publicadores sociais

## Diagnóstico
**Contexto Operacional:** Ranqueamento de credibilidade de publicadores sociais (TweetRank).
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-tweetrank`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Ranqueamento de credibilidade de publicadores sociais...");
    const executionResult = await performtweetrankLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-tweetrank",
        status: "SUCCESS",
...
async function performtweetrankLogic(payload: any) {
