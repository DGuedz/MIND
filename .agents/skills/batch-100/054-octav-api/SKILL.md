# Rastreio de histórico de transações e portfólios complexos

## Diagnóstico
**Contexto Operacional:** Rastreio de histórico de transações e portfólios complexos (Octav API).
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-octav-api`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Rastreio de histórico de transações e portfólios complexos...");
    const executionResult = await performoctavapiLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-octav-api",
        status: "SUCCESS",
...
async function performoctavapiLogic(payload: any) {
