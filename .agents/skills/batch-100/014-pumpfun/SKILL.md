# Lançamento instantâneo de tokens e trade em curvas de adesão

## Diagnóstico
**Contexto Operacional:** Lançamento instantâneo de tokens e trade em curvas de adesão (PumpFun).
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-pumpfun`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Lançamento instantâneo de tokens e trade em curvas de adesão...");
    const executionResult = await performpumpfunLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-pumpfun",
        status: "SUCCESS",
...
async function performpumpfunLogic(payload: any) {
