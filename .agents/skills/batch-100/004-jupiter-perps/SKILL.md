# Execução de trading de futuros perpétuos

## Diagnóstico
**Contexto Operacional:** Execução de trading de futuros perpétuos (Jupiter Perps).
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-jupiter-perps`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Execução de trading de futuros perpétuos...");
    const executionResult = await performjupiterperpsLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-jupiter-perps",
        status: "SUCCESS",
...
async function performjupiterperpsLogic(payload: any) {
