# Verificação de Prova de Humanidade para evitar ataques Sybil

## Diagnóstico
**Contexto Operacional:** Verificação de Prova de Humanidade para evitar ataques Sybil.
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-skill-27`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Verificação de Prova de Humanidade para evitar ataques Sybil...");
    const executionResult = await performskill27Logic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-skill-27",
        status: "SUCCESS",
...
async function performskill27Logic(payload: any) {
