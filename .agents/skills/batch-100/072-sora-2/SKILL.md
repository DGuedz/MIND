# Criação de vídeos com áudio e diálogo sincronizados

## Diagnóstico
**Contexto Operacional:** Criação de vídeos com áudio e diálogo sincronizados (Sora-2).
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-sora-2`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Criação de vídeos com áudio e diálogo sincronizados...");
    const executionResult = await performsora2Logic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-sora-2",
        status: "SUCCESS",
...
async function performsora2Logic(payload: any) {
