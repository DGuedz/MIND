# Análise gráfica OHLCV e dados de pools em tempo real

## Diagnóstico
**Contexto Operacional:** Análise gráfica OHLCV e dados de pools em tempo real (CoinGecko API).
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-coingecko-api`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Análise gráfica OHLCV e dados de pools em tempo real...");
    const executionResult = await performcoingeckoapiLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-coingecko-api",
        status: "SUCCESS",
...
async function performcoingeckoapiLogic(payload: any) {
