# Execução de criação de liquidez via Dynamic Bonding Curves

## Diagnóstico
**Contexto Operacional:** Execução de criação de liquidez via Dynamic Bonding Curves (Meteora).
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-meteora`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Execução de criação de liquidez via Dynamic Bonding Curves...");
    const executionResult = await performmeteoraLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-meteora",
        status: "SUCCESS",
...
async function performmeteoraLogic(payload: any) {
