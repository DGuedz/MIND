# Execução de swaps em ultra-baixa latência

## Diagnóstico
**Contexto Operacional:** Execução de swaps em ultra-baixa latência (Jupiter Ultra Swap).
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-jupiter-ultra-swap`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Execução de swaps em ultra-baixa latência...");
    const executionResult = await performjupiterultraswapLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-jupiter-ultra-swap",
        status: "SUCCESS",
...
async function performjupiterultraswapLogic(payload: any) {
