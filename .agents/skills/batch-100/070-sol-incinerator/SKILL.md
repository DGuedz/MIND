# Limpeza automatizada de carteiras com queima de tokens/NFTs de scam

## Diagnóstico
**Contexto Operacional:** Limpeza automatizada de carteiras com queima de tokens/NFTs de scam (SOL Incinerator).
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-sol-incinerator`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Limpeza automatizada de carteiras com queima de tokens/NFTs de scam...");
    const executionResult = await performsolincineratorLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-sol-incinerator",
        status: "SUCCESS",
...
async function performsolincineratorLogic(payload: any) {
