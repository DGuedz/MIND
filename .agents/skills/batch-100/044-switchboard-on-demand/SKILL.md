# Fornecimento de dados off-chain sob demanda

## Diagnóstico
**Contexto Operacional:** Fornecimento de dados off-chain sob demanda (Switchboard On-Demand).
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-switchboard-on-demand`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Fornecimento de dados off-chain sob demanda...");
    const executionResult = await performswitchboardondemandLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-switchboard-on-demand",
        status: "SUCCESS",
...
async function performswitchboardondemandLogic(payload: any) {
