# Depósitos e saques programados em Vaults de rendimento

## Diagnóstico
**Contexto Operacional:** Depósitos e saques programados em Vaults de rendimento (Kamino).
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-kamino`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Depósitos e saques programados em Vaults de rendimento...");
    const executionResult = await performkaminoLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-kamino",
        status: "SUCCESS",
...
async function performkaminoLogic(payload: any) {
