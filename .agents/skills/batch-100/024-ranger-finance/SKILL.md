# Agregação de perps cross-protocol para melhor taxa

## Diagnóstico
**Contexto Operacional:** Agregação de perps cross-protocol para melhor taxa (Ranger Finance).
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-ranger-finance`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Agregação de perps cross-protocol para melhor taxa...");
    const executionResult = await performrangerfinanceLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-ranger-finance",
        status: "SUCCESS",
...
async function performrangerfinanceLogic(payload: any) {
