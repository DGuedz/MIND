# Criação de ambientes de desenvolvimento clonando o estado da mainnet

## Diagnóstico
**Contexto Operacional:** Criação de ambientes de desenvolvimento clonando o estado da mainnet (Surfpool).
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-surfpool`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Criação de ambientes de desenvolvimento clonando o estado da mainnet...");
    const executionResult = await performsurfpoolLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-surfpool",
        status: "SUCCESS",
...
async function performsurfpoolLogic(payload: any) {
