# Roteamento de Swaps em pools CPMM/CLMM

## Diagnóstico
**Contexto Operacional:** Roteamento de Swaps em pools CPMM/CLMM (Raydium).
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-raydium`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Roteamento de Swaps em pools CPMM/CLMM...");
    const executionResult = await performraydiumLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-raydium",
        status: "SUCCESS",
...
async function performraydiumLogic(payload: any) {
