# Roteamento de yield lending otimizado multi-protocolo

## Diagnóstico
**Contexto Operacional:** Roteamento de yield lending otimizado multi-protocolo (Lulo).
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-lulo`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Roteamento de yield lending otimizado multi-protocolo...");
    const executionResult = await performluloLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-lulo",
        status: "SUCCESS",
...
async function performluloLogic(payload: any) {
