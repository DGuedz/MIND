# Execução de trading spot com captura de orderflow

## Diagnóstico
**Contexto Operacional:** Execução de trading spot com captura de orderflow (DFlow).
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-dflow`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Execução de trading spot com captura de orderflow...");
    const executionResult = await performdflowLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-dflow",
        status: "SUCCESS",
...
async function performdflowLogic(payload: any) {
