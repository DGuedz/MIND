...
## Diagnóstico
**Contexto Operacional:** Execução de zk-KYC (provas de conhecimento zero) para privacidade em mercados regulados.
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-provas-de-conhecimento-zero`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Execução de zk-KYC...");
    const executionResult = await performprovasdeconhecimentozeroLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-provas-de-conhecimento-zero",
        status: "SUCCESS",
...
async function performprovasdeconhecimentozeroLogic(payload: any) {
