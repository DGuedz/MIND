# Monitoramento de trending topics globais por geolocalização

## Diagnóstico
**Contexto Operacional:** Monitoramento de trending topics globais por geolocalização.
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-skill-52`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Monitoramento de trending topics globais por geolocalização...");
    const executionResult = await performskill52Logic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-skill-52",
        status: "SUCCESS",
...
async function performskill52Logic(payload: any) {
