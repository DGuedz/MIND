# Validação reutilizável de KYC e integração com Sumsub ID

## Diagnóstico
**Contexto Operacional:** Validação reutilizável de KYC e integração com Sumsub ID.
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-skill-31`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Validação reutilizável de KYC e integração com Sumsub ID...");
    const executionResult = await performskill31Logic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-skill-31",
        status: "SUCCESS",
...
async function performskill31Logic(payload: any) {
