# Provisão e rebalanceamento de liquidez concentrada

## Diagnóstico
**Contexto Operacional:** Provisão e rebalanceamento de liquidez concentrada (Orca Whirlpools).
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-orca-whirlpools`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Provisão e rebalanceamento de liquidez concentrada...");
    const executionResult = await performorcawhirlpoolsLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-orca-whirlpools",
        status: "SUCCESS",
...
async function performorcawhirlpoolsLogic(payload: any) {
