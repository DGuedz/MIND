# Swaps arbitráveis de Liquid Staking Tokens

## Diagnóstico
**Contexto Operacional:** Swaps arbitráveis de Liquid Staking Tokens (Sanctum).
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-sanctum`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Swaps arbitráveis de Liquid Staking Tokens...");
    const executionResult = await performsanctumLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-sanctum",
        status: "SUCCESS",
...
async function performsanctumLogic(payload: any) {
