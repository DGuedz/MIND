# Trading alavancado de RWAs, commodities e memecoins

## Diagnóstico
**Contexto Operacional:** Trading alavancado de RWAs, commodities e memecoins (Lavarage).
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-lavarage`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Trading alavancado de RWAs, commodities e memecoins...");
    const executionResult = await performlavarageLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-lavarage",
        status: "SUCCESS",
...
async function performlavarageLogic(payload: any) {
