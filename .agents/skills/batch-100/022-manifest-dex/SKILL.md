# Leitura de mercado e colocação de ordens em CLOB

## Diagnóstico
**Contexto Operacional:** Leitura de mercado e colocação de ordens em CLOB (Manifest DEX).
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-manifest-dex`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Leitura de mercado e colocação de ordens em CLOB...");
    const executionResult = await performmanifestdexLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-manifest-dex",
        status: "SUCCESS",
...
async function performmanifestdexLogic(payload: any) {
