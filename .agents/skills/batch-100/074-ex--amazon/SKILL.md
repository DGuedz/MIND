# Compras autônomas em marketplaces

## Diagnóstico
**Contexto Operacional:** Compras autônomas em marketplaces (ex: Amazon) pagas nativamente em USDC (SP3ND).
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-ex--amazon`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Compras autônomas em marketplaces...");
    const executionResult = await performexamazonLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-ex--amazon",
        status: "SUCCESS",
...
async function performexamazonLogic(payload: any) {
