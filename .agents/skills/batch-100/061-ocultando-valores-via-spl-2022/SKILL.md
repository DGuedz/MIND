...
## Diagnóstico
**Contexto Operacional:** Transferências Confidenciais (ocultando valores via SPL-2022).
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-ocultando-valores-via-spl-2022`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Transferências Confidenciais...");
    const executionResult = await performocultandovaloresviaspl2022Logic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-ocultando-valores-via-spl-2022",
        status: "SUCCESS",
...
async function performocultandovaloresviaspl2022Logic(payload: any) {
