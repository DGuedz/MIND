# Emissão de Notas de Participação de Receita

## Diagnóstico
**Contexto Operacional:** Emissão de Notas de Participação de Receita (Rev-share notes) com custódia autônoma.
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-rev-share-notes`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Emissão de Notas de Participação de Receita...");
    const executionResult = await performrevsharenotesLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-rev-share-notes",
        status: "SUCCESS",
...
async function performrevsharenotesLogic(payload: any) {
