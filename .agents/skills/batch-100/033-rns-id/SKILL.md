# Verificação de identidade digital e residência on-chain

## Diagnóstico
**Contexto Operacional:** Verificação de identidade digital e residência on-chain (RNS.ID).
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-rns-id`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Verificação de identidade digital e residência on-chain...");
    const executionResult = await performrnsidLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-rns-id",
        status: "SUCCESS",
...
async function performrnsidLogic(payload: any) {
