# Inscrições digitais criadas a partir de entropia on-chain nativa

## Diagnóstico
**Contexto Operacional:** Inscrições digitais criadas a partir de entropia on-chain nativa (Sentients).
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-sentients`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Inscrições digitais criadas a partir de entropia on-chain nativa...");
    const executionResult = await performsentientsLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-sentients",
        status: "SUCCESS",
...
async function performsentientsLogic(payload: any) {
