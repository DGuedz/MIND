# Tokenização de Ativos do Mundo Real em RWA com atrelamento à identidades civis

## Diagnóstico
**Contexto Operacional:** Tokenização de Ativos do Mundo Real em RWA com atrelamento à identidades civis.
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-skill-91`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Tokenização de Ativos do Mundo Real em RWA com atrelamento à identidades civis...");
    const executionResult = await performskill91Logic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-skill-91",
        status: "SUCCESS",
...
async function performskill91Logic(payload: any) {
