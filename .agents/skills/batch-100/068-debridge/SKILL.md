# Orquestração de mensageria e transferências cross-chain EVM-Solana

## Diagnóstico
**Contexto Operacional:** Orquestração de mensageria e transferências cross-chain EVM-Solana (deBridge).
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-debridge`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Orquestração de mensageria e transferências cross-chain EVM-Solana...");
    const executionResult = await performdebridgeLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-debridge",
        status: "SUCCESS",
...
async function performdebridgeLogic(payload: any) {
