# Extração de Médias Móveis Exponenciais on-chain

## Diagnóstico
**Contexto Operacional:** Extração de Médias Móveis Exponenciais on-chain (EMA via Pyth).
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-ema-via-pyth`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Extração de Médias Móveis Exponenciais on-chain...");
    const executionResult = await performemaviapythLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-ema-via-pyth",
        status: "SUCCESS",
...
async function performemaviapythLogic(payload: any) {
