# Orquestração de fluxos para Neobanks Regionais e Remessas FX em Stablecoins

## Diagnóstico
**Contexto Operacional:** Orquestração de fluxos para Neobanks Regionais e Remessas FX em Stablecoins.
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-skill-92`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Orquestração de fluxos para Neobanks Regionais e Remessas FX em Stablecoins...");
    const executionResult = await performskill92Logic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-skill-92",
        status: "SUCCESS",
...
async function performskill92Logic(payload: any) {
