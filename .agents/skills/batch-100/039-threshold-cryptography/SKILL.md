# Assinaturas Multi-party

## Diagnóstico
**Contexto Operacional:** Assinaturas Multi-party (Threshold Cryptography) para destrancar vaults.
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-threshold-cryptography`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Assinaturas Multi-party...");
    const executionResult = await performthresholdcryptographyLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-threshold-cryptography",
        status: "SUCCESS",
...
async function performthresholdcryptographyLogic(payload: any) {
