# Provisão dinâmica de liquidez

## Diagnóstico
**Contexto Operacional:** Provisão dinâmica de liquidez (Meteora DLMM).
...
## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-meteora-dlmm`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Provisão dinâmica de liquidez...");
    const executionResult = await performmeteoradlmmLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-meteora-dlmm",
        status: "SUCCESS",
...
async function performmeteoradlmmLogic(payload: any) {
