# Leitura de metadados nativos para "Zero-Trust Payload"

## Diagnóstico
**Contexto Operacional:** Leitura de metadados nativos para "Zero-Trust Payload" (onde o preço vem do blockchain e não do agente).
Esta skill foi projetada para atuar na economia máquina-a-máquina (A2A), encapsulando lógica de Meta-Skill complexa através de um firewall Zero-Trust.

## Intent Workflow
1. **Intent:** Agente despacha a intenção de uso para `mind-skill-onde-o-pre-o-vem-do-blockchain-e-n-o-do-agente`.
...
export async function executeSkill(intentPayload: any, agentPubkey: PublicKey) {
    ...
    console.log("Executando: Leitura de metadados nativos para "Zero-Trust Payload"...");
    const executionResult = await performondeopreovemdoblockchainenodoagenteLogic(intentPayload);

    // 4. Emissão do Mindprint (Proof-Native Delivery)
    const receipt = await Mindprint.issue({
        agent: agentPubkey,
        skillId: "mind-skill-onde-o-pre-o-vem-do-blockchain-e-n-o-do-agente",
        status: "SUCCESS",
...
async function performondeopreovemdoblockchainenodoagenteLogic(payload: any) {
