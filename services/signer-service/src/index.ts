...
const signRequestSchema = z.object({
  ...
  // Only Agents Vision: Credential Gating
  agentId: z.string().optional(),
  credentialNft: z.string().optional(),
});
...
app.post("/v1/signer/sign", async (request, reply) => {
  try {
    ...
    // 1. Only Agents Validation: Credential Check
    if (parsed.agentId && parsed.credentialNft) {
      request.log.info({ agentId: parsed.agentId, nft: parsed.credentialNft }, "Validating Agent Credential before signing");
      // Placeholder: In production, verify ownership of the Metaplex cNFT here.
      // If unauthorized: return reply.status(403).send({ error: "UNAUTHORIZED_AGENT_CREDENTIAL" });
    }
